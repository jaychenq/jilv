...
MIME::Types['application/octet-stream'][0].extensions.unshift('')
...
module Paperclip
  module ClassMethods
    ...
    def has_attached_file_with_patch(name, options = {})
      self.cattr_accessor *(["#{name}_attachment"] + %w[file_name file_size content_type updated_at image_width image_height].map { |field| "#{name}_attachment_#{field}" })
      
      # has_attached_file("#{name}_attachment", options)
      HasAttachedFile.define_on(self, "#{name}_attachment", options)
      
      define_method "#{name}=" do |file|
        if file.respond_to?(:read)
          # if geometry = Paperclip::Geometry.from_file(file) rescue nil
          #   self["#{name}_attachment_image_width"] = geometry.width if self.respond_to?("#{name}_attachment_image_width")
          #   self["#{name}_attachment_image_height"] = geometry.height if self.respond_to?("#{name}_attachment_image_height")
          # end
          self.send("#{name}_attachment=", file)
          self[name] = self.send("#{name}_attachment").url(:original, false)
          logger.info self.send("#{name}_attachment")
      ...
      define_method "before_#{name}_post_process" do; end
    ...
    alias_method_chain :has_attached_file, :patch
  end
end
...
module Paperclip
  module Validators
    ...
    module ClassMethods
      ...
      alias_method_chain :validates_attachment, :patch
    end
  end
end
