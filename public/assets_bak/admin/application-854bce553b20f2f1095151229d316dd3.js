...
ga('create', 'UA-49529714-1', 'auto', { 'allowLinker': true });
...
(function() {
  $(function() {
    return $(document).on('page:change', function() {
      ...
      return $("input.datetime, input[name$='_at]']").dynDateTime({
        showsTime: true,
        showOthers: true,
        firstDay: 1,
    ...
    return $(document).on('page:change', function() {
      ...
      return $.ajax({
        ...
        tinymce.init({
          ...
          theme: "modern",
          height: 200,
          plugins: ["advlist autolink link image lists charmap preview hr pagebreak", "searchreplace visualblocks visualchars code fullscreen insertdatetime nonbreaking", "save contextmenu directionality paste textcolor"],
          toolbar: "insertfile undo redo | styleselect | bold italic | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link image | preview | forecolor backcolor"
        });
...
(function() {
  $(function() {
    return $(document).on('page:change', function() {
      return $('.publish_button').on('click', function() {
        if (!$(this).hasClass('enable') || $(this).hasClass('loading')) {
          return false;
        }
        $.ajax({
          type: 'PUT',
          url: $(this).data().root + '/' + $(this).data().id + '/' + ($(this).hasClass('active') ? 'withdraw' : 'publish'),
          context: this,
          beforeSend: function() {
    ...
    return $(document).on('page:change', function() {
      return $('#search_form').submit(function() {
        ...
        return setTimeout((function() {
          return $(that).find(':input').attr('disabled', false);
        }), 500);
      });
    });
  });
